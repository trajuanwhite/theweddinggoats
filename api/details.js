...
export default async function handler(req, res) {
  ...
  try {
    ...
    let attachment = null;
    if (data.timelineAttachment) {
      const { name, type, contentBytes } = data.timelineAttachment;
      const fileName = clean(name);
      const mimeType = clean(type);
      const base64 = clean(contentBytes);
      if (!fileName || !base64 || !allowedTypes.has(mimeType)) return res.status(400).json({ok:false,error:'Unsupported timeline file'});
      const estimatedBytes = Math.ceil((base64.length * 3) / 4);
      if (estimatedBytes > 2 * 1024 * 1024) return res.status(400).json({ok:false,error:'Timeline file too large'});
      attachment = {
