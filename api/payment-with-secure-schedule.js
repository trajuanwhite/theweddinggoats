...
async function sendPaymentSchedule({ name, email, weddingDate, packageKey, paymentId }) {
  ...
  const bookingKey = createBalanceKey(paymentId, email);
  if (!bookingKey) return false;
  const balanceUrl = `https://theweddinggoats.com/balance-secure.html?key=${encodeURIComponent(bookingKey)}`;
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;color:#1e1d1a;line-height:1.65;max-width:680px;margin:auto">
    ...
    <p>When you're ready, use the button below. Your booking is securely connected to this link, so you won't need to enter a payment code or re-enter your email.</p>
    ...
    <p style="font-size:13px;color:#6c655d">If you ever open the balance page without this email, the manual booking lookup will still be available as a backup.</p>
    ...
    async json(payload) {
      if (statusCode >= 200 && statusCode < 300 && payload?.ok && payload?.paymentId) {
        try {
          ...
          console.error('Secure payment schedule email error', error?.message || error);
        }
      }
